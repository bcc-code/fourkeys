...
(ARRAY_AGG(title IGNORE NULLS))[OFFSET(0)] title,
(ARRAY_AGG(url IGNORE NULLS))[OFFSET(0)] url,
ARRAY_AGG(root_cause IGNORE NULLS) changes
FROM
(
SELECT 
source,
CASE WHEN source LIKE "github%" THEN JSON_EXTRACT_SCALAR(metadata, '$.issue.title') end as title,
CASE WHEN source LIKE "github%" THEN JSON_EXTRACT_SCALAR(metadata, '$.issue.html_url') end as url,
CASE WHEN source LIKE "github%" THEN id # JSON_EXTRACT_SCALAR(metadata, '$.issue.id')
